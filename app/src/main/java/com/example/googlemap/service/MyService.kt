...
//                Log.d(TAG, "getLastLocation: ${it.longitude}")
                ...
                location.longtitude = it.longitude
                location.time = time
                myDatabase.myDao().addTime(location)
...
//                }
                val myDatabase = MyDatabase.getInstance(this@MyService)
                val location = MyLocation()
                val time = SimpleDateFormat("dd.MM.yyyy HH:mm:ss").format(Date())
                location.latitude = currentLocation!!.latitude
                location.longtitude = currentLocation!!.longitude
                location.time = time
                myDatabase.myDao().addTime(location)
            }
        }
